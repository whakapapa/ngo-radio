...
RadiotrayNG::RadiotrayNG(std::shared_ptr<IConfig> config, std::shared_ptr<IBookmarks> bookmarks,
	...
	{
		this->notification_image = radiotray_ng::word_expand(this->config->get_string(RADIOTRAY_NG_NOTIFICATION_KEY, DEFAULT_RTRAY_NOTIFICATION_VAL));

		this->set_volume(this->config->get_string(VOLUME_LEVEL_KEY, std::to_string(DEFAULT_VOLUME_LEVEL_VAL)));
	...
	void RadiotrayNG::stop()
	{
		...
		if (this->config->get_bool(NOTIFICATION_KEY, DEFAULT_NOTIF_VAL) &&
		this->config->get_bool(NOTIFICATION_VERBOSE_KEY, DEFAULT_NOTIF_VERBOSE_VAL))
		{
	...
	void RadiotrayNG::set_volume_msg(const uint32_t volume)
	{
		this->set_and_save_volume(std::min(volume,
			this->config->get_uint32(VOLUME_MAX_LEVEL_KEY, DEFAULT_VOLUME_LEVEL_MAX_VAL)));
		...
		{
			if (this->config->get_bool(SPLIT_TITLE_KEY, DEFAULT_SPLIT_TITLE_VAL))
			{
				if (data.count(TAG_TITLE) && !data[TAG_TITLE].empty())
				{
					...
					if (pos != std::string::npos)
					{
						...
						else
						{
							...
							if (this->config->get_bool(IHR_TITLE_KEY, DEFAULT_IHR_TITLE_KEY_VAL))
							{
		...
		void RadiotrayNG::on_state_changed_event(const IEventBus::event& /*ev*/, IEventBus::event_data_t& data)
		{
			...
			if (this->config->get_bool(NOTIFICATION_KEY, DEFAULT_NOTIF_VAL))
			{
				...
				if (this->config->get_bool(NOTIFICATION_VERBOSE_KEY, DEFAULT_NOTIF_VERBOSE_VAL))
				{
		...
		void RadiotrayNG::on_message_event(const IEventBus::event& /*ev*/, IEventBus::event_data_t &data)
		{
			if (this->config->get_bool(NOTIFICATION_VERBOSE_KEY, DEFAULT_NOTIF_VERBOSE_VAL))
			{
				this->notification.notify(data[MESSAGE_KEY], APP_NAME_DISPLAY,
					radiotray_ng::word_expand(this->config->get_string(RADIOTRAY_NG_NOTIFICATION_KEY, DEFAULT_RTRAY_NOTIFICATION_VAL)));
				}
			}
			...
			void RadiotrayNG::on_tags_changed_event_notification(const IEventBus::event& /*ev*/, IEventBus::event_data_t& data)
			{
				if (data.count(TAG_TITLE))
				{
					...
					{
						const uint32_t max_volume{this->config->get_uint32(VOLUME_MAX_LEVEL_KEY, DEFAULT_VOLUME_LEVEL_MAX_VAL)};
						const uint32_t volume{this->config->get_uint32(VOLUME_LEVEL_KEY, DEFAULT_VOLUME_LEVEL_VAL)};

						this->set_and_save_volume(std::min(volume + this->config->get_uint32(VOLUME_STEP_KEY, DEFAULT_VOLUME_STEP_VAL), max_volume));
					}
					...
					{
						const uint32_t volume_step(this->config->get_uint32(VOLUME_STEP_KEY, DEFAULT_VOLUME_STEP_VAL));
						const uint32_t volume(this->config->get_uint32(VOLUME_LEVEL_KEY, DEFAULT_VOLUME_LEVEL_VAL));
					...
					void RadiotrayNG::display_volume_level()
					{
						...
						std::string volume_str = "Volume: " + std::to_string(this->config->get_uint32(VOLUME_LEVEL_KEY, DEFAULT_VOLUME_LEVEL_VAL)) + "%";
