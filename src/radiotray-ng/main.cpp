...
{
	if (config->get_bool(DEBUG_LOGGING_KEY, DEFAULT_DEBUG_LOGGING_VAL))
	{
...
void set_config_defaults(std::shared_ptr<IConfig> config, const std::string& config_path)
{
	...
	config->set_bool(NOTIFICATION_KEY, DEFAULT_NOTIF_VAL);
	config->set_bool(SPLIT_TITLE_KEY, DEFAULT_SPLIT_TITLE_VAL);
	config->set_uint32(VOLUME_LEVEL_KEY, DEFAULT_VOLUME_LEVEL_VAL);
	config->set_bool(NOTIFICATION_VERBOSE_KEY, DEFAULT_NOTIF_VERBOSE_VAL);
...
{
	bookmarks->add_group(ROOT_BOOKMARK_GROUP, DEFAULT_STATION_IMAGE_VAL);
}
