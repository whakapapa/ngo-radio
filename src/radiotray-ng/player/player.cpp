...
bool Player::play_next()
{
	...
	if (!this->current_playlist.empty())
	{
		...
		uint32_t buffer_size = this->config->get_uint32(BUFFER_SIZE_KEY, DEFAULT_BUFFER_SIZE_VAL);
		uint32_t buffer_duration = this->config->get_uint32(BUFFER_DURATION_KEY, DEFAULT_BUFFER_DURATION_VAL);
		...
		this->volume(this->config->get_uint32(VOLUME_LEVEL_KEY, DEFAULT_VOLUME_LEVEL_VAL));
