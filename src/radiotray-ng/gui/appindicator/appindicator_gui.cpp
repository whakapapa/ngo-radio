...
AppindicatorGui::AppindicatorGui(std::shared_ptr<IConfig> config, std::shared_ptr<IRadioTrayNG> radiotray_ng,
	...
	, file_monitor_timer_id(0)
	{
		...
		if (this->config->get_bool(FILE_MONITOR_KEY, DEFAULT_FILE_MONITOR_VAL))
		{
			...
			this->file_monitor_timer_id = g_timeout_add(this->config->get_uint32(FILE_MONITOR_INTERVAL_KEY, DEFAULT_FILE_MONITOR_INT_VAL) * 1000,
					...
					void AppindicatorGui::on_state_event(const IEventBus::event& /*ev*/, IEventBus::event_data_t& data)
					{
						...
						if (state == STATE_PLAYING || state == STATE_BUFFERING || state == STATE_CONNECTING)
						{
							app_indicator_set_icon(this->appindicator, radiotray_ng::word_expand(this->config->get_string(RADIOTRAY_NG_ICON_ON_KEY,
								DEFAULT_RTRAY_ICON_ON_VAL)).c_str());
								return;
							}
							...
							if (state == STATE_STOPPED)
							{
								app_indicator_set_icon(this->appindicator, radiotray_ng::word_expand(this->config->get_string(RADIOTRAY_NG_ICON_OFF_KEY,
									DEFAULT_RTRAY_ICON_OFF_VAL)).c_str());
									return;
								}
							}
							...
							void AppindicatorGui::build_bookmarks_menu_item()
							{
								...
								const bool root_bottom = this->config->get_bool(ROOT_GROUP_BOTTOM_POS_KEY, DEFAULT_ROOT_GRP_BOTTOM_POS_VAL);
							...
							{
								if (!this->config->get_bool(COMPACT_MENU_KEY, DEFAULT_COMPACT_MENU_VAL))
								{
							...
							void AppindicatorGui::update_volume_menu_item()
							{
								...
								if (this->config->get_bool(TAG_INFO_VERBOSE_KEY, DEFAULT_TAG_INFO_VERBOSE_VAL))
								{
							...
							void AppindicatorGui::update_action_menu_item(const std::string& state)
							{
								...
								{
									const bool copy_enabled = this->config->get_bool(TRACK_INFO_COPY_KEY, DEFAULT_TRACK_INFO_COPY_VAL);
									...
									if (state == STATE_PLAYING)
									{
										...
										{
											const bool wrap_enabled = this->config->get_bool(WRAP_TRACK_INFO_KEY, DEFAULT_WRAP_TRACK_INFO_VAL);

											const uint32_t wrap_len = this->config->get_uint32(WRAP_TRACK_INFO_LEN_KEY, DEFAULT_WRAP_TRACK_INFO_LEN_VAL);
								...
								void AppindicatorGui::build_status_menu_item()
								{
									...
									if (this->config->get_bool(TRACK_INFO_COPY_KEY, DEFAULT_TRACK_INFO_COPY_VAL))
									{
								...
								void AppindicatorGui::build_menu()
								{
									...
									if (!this->config->get_bool(INVERT_MENU_KEY, DEFAULT_INVERT_MENU_VAL))
									{
										// action (play/stop)
								...
								bool AppindicatorGui::sleep_timer_dialog()
								{
									...
									gtk_entry_set_text(GTK_ENTRY(entry), this->config->get_string(SLEEP_TIMER_KEY, std::to_string(DEFAULT_SLEEP_TIMER_VAL)).c_str());
									...
									else
									{
										if (app->sleep_timer_dialog())
										{
											app->event_bus->publish_only(IEventBus::event::message, MESSAGE_KEY,
												std::to_string(app->config->get_uint32(SLEEP_TIMER_KEY, DEFAULT_SLEEP_TIMER_VAL)) + " minute sleep timer started");

												app->sleep_timer_id = g_timeout_add(
													app->config->get_uint32(SLEEP_TIMER_KEY, DEFAULT_SLEEP_TIMER_VAL) * 60000, on_timer_event, data);
												}
												else
												{
										...
										void AppindicatorGui::on_reload_bookmarks_menu_item(GtkWidget* /*widget*/, gpointer data)
										{
											...
											{
												// reset file monitor...
												if (app->config->get_bool(FILE_MONITOR_KEY, DEFAULT_FILE_MONITOR_VAL))
												{
										...
										void AppindicatorGui::on_about_menu_item(GtkWidget* /*widget*/, gpointer /*data*/)
										{
											...
											void AppindicatorGui::run(int argc, char* argv[])
											{
												...
												const std::string icon_off{radiotray_ng::word_expand(this->config->get_string(RADIOTRAY_NG_ICON_OFF_KEY, DEFAULT_RTRAY_ICON_OFF_VAL))};
