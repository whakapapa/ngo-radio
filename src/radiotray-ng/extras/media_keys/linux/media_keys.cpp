...
public:
		...
		, dbus_proxy(nullptr)
	{
		...
		if (this->config->get_bool(MEDIA_KEY_MAPPING_KEY, DEFAULT_M_KEY_MAPPING_VAL))
		{
			this->media_keys[radiotray_ng::to_lower(this->config->get_string(MEDIA_KEY_VOLUME_UP_KEY, DEFAULT_M_KEY_VOLUME_UP_VAL))] =
				std::bind(&IRadioTrayNG::volume_up_msg, this->radiotray_ng);

			this->media_keys[radiotray_ng::to_lower(this->config->get_string(MEDIA_KEY_VOLUME_DOWN_KEY, DEFAULT_M_KEY_VOLUME_DOWN_VAL))] =
				std::bind(&IRadioTrayNG::volume_down_msg, this->radiotray_ng);

			this->media_keys[radiotray_ng::to_lower(this->config->get_string(MEDIA_KEY_NEXT_STAITON_KEY, DEFAULT_M_KEY_NEXT_STATION_VAL))] =
				std::bind(&IRadioTrayNG::next_station_msg, this->radiotray_ng);

			this->media_keys[radiotray_ng::to_lower(this->config->get_string(MEDIA_KEY_PREVIOUS_STATION_KEY, DEFAULT_M_KEY_PREV_STATION_VAL))] =
		...
		{
			if (this->config->get_bool(MEDIA_KEY_OLD_DBUS_NAME_KEY,	DEFAULT_M_KEY_OLD_DBUS_NAME_VAL))
			{
				this->dbus_name = "org.gnome.SettingsDaemon";
			}
		}
	...
	void log_media_keys()
	{
		LOG(info) << "mapping volume up/down to: "<< this->config->get_string(MEDIA_KEY_VOLUME_UP_KEY, DEFAULT_M_KEY_VOLUME_UP_VAL) << ", "
			<< this->config->get_string(MEDIA_KEY_VOLUME_DOWN_KEY, DEFAULT_M_KEY_VOLUME_DOWN_VAL);

		LOG(info) << "mapping station previous/next to: "<< this->config->get_string(MEDIA_KEY_PREVIOUS_STATION_KEY, DEFAULT_M_KEY_PREV_STATION_VAL) << ", "
			<< this->config->get_string(MEDIA_KEY_NEXT_STAITON_KEY, DEFAULT_M_KEY_NEXT_STATION_VAL);
	}
...
void media_keys_t::on_gio_signal(GDBusProxy* /*proxy*/, gchar* /*sender_name*/, gchar* /*signal_name*/, GVariant* parameters, gpointer user_data)
{
	...
	if (media_keys->config->get_bool(MEDIA_KEY_MAPPING_KEY, DEFAULT_M_KEY_MAPPING_VAL))
	{
